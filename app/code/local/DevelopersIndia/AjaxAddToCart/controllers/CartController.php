...
class DevelopersIndia_AjaxAddToCart_CartController extends Mage_Checkout_CartController
{
	...
	public function addAction()
    {
        ...
        try {
           ...
           $message .= '<div class="floatLeftAjax"><img src="'.$product->getImageUrl().'" class="imgUrlAjax"></div>';
 		   $message .="<div class='floatRightAjax'>";
 		   $message .="<p class='addedStuf'>";
           $message .= $this->__('%s<br />was successfully added to your shopping cart.', "<b>".$product->getName()."</b>");
         
           $temp_message ="</p><div class='clear'></div>
           <div class='recentStuff'>Recently added deals</div>
           <ul>";
           $item_added=0;
           $quote = Mage::helper('checkout')->getQuote();
			...
			// output details of an item.
				if($item->getProductId()!=$product->getId()){
						$temp_message .='<li>'. $item->getName().' x ('.$item->getQty().')</li>';
					$item_added++;
				}
			}

		   $temp_message	.="</ul></div><div class='clear'></div>";
		   if($item_added>0)
		   	$message .=$temp_message;
		   else
		   	$message .="</p><div class='clear'></div></div><div class='clear'></div>";
			// if(isset($params['in_cart']) && $params['in_cart']!=0){
			if($this->getRequest()->getParam('in_cart')){
				$this->getRequest()->setParam('from', 'checkout');
				$this->_forward('index');
				// $this->loadLayout('ajaxaddtocart_cart_index');
				// header('Content-type: application/json');
				// $return['r'] = 'success';
	...
	public function sidebardeleteAction(){
		...
		$return['r'] = 'success';
		// $return['cart'] = $this->getLayout()->createBlock('checkout/cart_sidebar')->setTemplate('checkout/cart/sidebar.phtml')->toHtml();
		if($this->getRequest()->getParam('from')=="checkout"){
			// $this->loadLayout('ajaxaddtocart_cart_index');
			// $return['big_cart'] = $this->getLayout()->getBlock('content')->toHtml();
			$this->_forward('index');
		}else{
    ...
    public function configureAction()
    {
        ...
        try {
			...
			$layoutHandle = '';
			// echo $quoteItem->getProduct()->getTypeId()."<-- type id";
			if(!in_array($quoteItem->getProduct()->getTypeId(), $allowedProductTypes)){
				...
				if(count($quoteItem->getProduct()->getOptions())>0){
					$layoutHandle = 'ajaxaddtocart_cart_configure_'.$quoteItem->getProduct()->getTypeId();
				}
			}
			// echo $layoutHandle."<--layoutHandle<br/>";
			$this->loadLayout($layoutHandle);
    ...
    public function updateItemOptionsAction()
    {
        ...
        try {
			...
			if($this->getRequest()->getParam('from')){
				if($this->getRequest()->getParam('from')=="checkout"){
					// $this->loadLayout('ajaxaddtocart_cart_index');
					// $this->getLayout('ajaxaddtocart_cart_index');
					// $this->getLayout()->getUpdate()->addHandle('ajaxaddtocart_cart_index');
