...
class Gclone_Advertsystem_Model_Observer {
                ...
                if ($discountType == 2) {
                    ...
                    if ($discountTotalAmount < $MaxpercentAmount) {
                        if (count($discountRecord) > 0) {
                            ...
                            $executeQuery = $read->query("insert into $discountTable values ('',$custId,0,0,$discountTotalAmount,0,0,$dicountRemaining,1)");
                        }
                    }
                } else {
    ...
    public function processUpdateDiscount($observer) {
        ...
        if (count($fetchCustomer) > 0 && $discountQuoteAmount != 0 && $status == 'processing' || $status == 'complete') {
            $up = $read->query("UPDATE $discountTable SET order_id = $orderId, previous_amount = $previousAmount, $setField WHERE discount_id =" . $discountId);
    ...
    {       
        ...
        $up = $read->query("UPDATE $discountTable SET previous_amount = 0, amount = $totalAmount WHERE order_id =" . $orderId);
    }
