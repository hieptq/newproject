    ...
    public function giftPostAction() {
        ...
        if ($customerId != '') {
            ...
            if (isset($getGiftDetail['email'])) {
                $write->query("UPDATE " . $tprefix . "gift_message set sender ='" . addslashes($fromName) . "',recipient='" . addslashes($toName) . "',message='" . addslashes($message) . "',email='" . $email . "' where customer_id = '" . $customerId . "' AND product_id ='$productId' AND order_id = 0");
                $this->_getSession()->addSuccess($this->__('Gift was Updated successfully.'));
            } else {
                $write->query("insert into " . $tprefix . "gift_message (customer_id,sender,recipient,message,email,product_id)values ($customerId,'" . addslashes($fromName) . "','" . addslashes($toName) . "','" . addslashes($message) . "','" . $email . "','" . $productId . "')");
